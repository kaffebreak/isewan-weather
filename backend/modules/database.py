...
# Applied when a caller (e.g. CSV export) omits `limit` entirely. Without a
# cap, GET /api/weather/data would return the whole table in one response.
...
class WeatherDatabase:
    ...
    @contextmanager
    def _connect(self):
        """Yield a connection, guaranteeing it's closed even on error."""
    ...
    def init_database(self):
        with self._connect() as conn:
            ...
            # which matters once the server handles requests on multiple
            # threads.
    ...
    def get_weather_data(
        ...
        if limit is not None:
            ...
            # No explicit page size (e.g. a CSV export request): still cap
            # the response instead of returning the entire table.
